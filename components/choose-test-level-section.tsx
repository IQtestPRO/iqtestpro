...
export default function ChooseTestLevelSection() {
  ...
  const validateTestContent = (testId: number) => {
    const question = sampleQuestionsData.find((q) => q.id === testId)
    if (!question) return { isValid: false, errors: ["Test not found"] }

    const errors: string[] = []

    // Validate question count matches promise
    if (testId === 1 && question.questions !== 15) {
      errors.push("Question count mismatch: Expected 15 for Novato level")
    }
    if (testId === 2 && question.questions !== 20) {
      errors.push("Question count mismatch: Expected 20 for Pro level")
    }
    if (testId === 3 && question.questions !== 25) {
      errors.push("Question count mismatch: Expected 25 for Elite level")
    }
    if (testId === 4 && question.questions !== 50) {
      errors.push("Question count mismatch: Expected 50 for Lenda level")
    }

    // Validate features match promises
    const requiredFeatures = {
      1: ["Relatório de inteligência espacial", "Badge de conquista", "Suporte 24/7"],
      2: ["Relatório de inteligência lógica", "Badge de conquista", "Ranking global", "Suporte premium"],
      3: [
        "Relatório psicométrico completo",
        "Badge de conquista",
      ...
      4: ["Relatório master completo", "Badge de conquista", "Análise de 8 inteligências", "Plano de desenvolvimento"],
    }

    const required = requiredFeatures[testId as keyof typeof requiredFeatures] || []
    const missing = required.filter(
      (feature) => !question.features.some((f) => f.toLowerCase().includes(feature.toLowerCase().split(" ")[0])),
    )

    if (missing.length > 0) {
      errors.push(`Missing promised features: ${missing.join(", ")}`)
    }
