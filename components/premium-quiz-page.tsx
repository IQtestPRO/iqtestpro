...
import { useRouter } from "next/navigation" // Usar useRouter diretamente, o level virá via props
...
export default function PremiumQuizPage({ level }: PremiumQuizPageProps) {
  ...
  const handleStartQuiz = () => {
    try {
      ...
      setSelectedAnswer(null) // Reset selected answer on start
      setIsStarted(true)
    } catch (error) {
  ...
  const handlePreviousQuestion = () => {
    ...
    if (quizEngine.previousQuestion()) {
      setCurrentQuestion(quizEngine.getCurrentQuestion())
      setSelectedAnswer(quizEngine.getUserAnswer(quizEngine.getCurrentQuestionIndex())) // Load previous answer
    }
  }
